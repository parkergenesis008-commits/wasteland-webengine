...
想象你把一张藏宝图撕成碎片，分给100个人每人一块。如果其中10个人走丢了，你只能找回90块碎片——你丢失了10%的信息。但如果你在分出去之前，先把藏宝图复印100份，再全部撕碎混在一起分发——每个碎片几乎包含了完整地图的信息。这时候丢了10个人，你几乎什么都不损失。

这就是"scrambling"（量子搅乱）做的事：它不是把信息分散在局部，而是均匀地涂抹在整个系统的**关联结构**中。每个粒子都包含了关于全局参数的信息——不是局部信息，而是**体积律**分布的信息。
